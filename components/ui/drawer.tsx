...
const DrawerOverlay = React.forwardRef<React.ElementRef<typeof DrawerPrimitive.Overlay>, React.ComponentPropsWithoutRef<typeof DrawerPrimitive.Overlay>>(
  ({ className, ...props }, ref) => (
    <DrawerPrimitive.Overlay ref={ref} className={cn("fixed inset-0 border-none border-neutral-950 z-20 bg-black/80", className)} {...props} />
  )
)
...
const DrawerContent = React.forwardRef<React.ElementRef<typeof DrawerPrimitive.Content>, React.ComponentPropsWithoutRef<typeof DrawerPrimitive.Content>>(
  ({ className, children, ...props }, ref) => (
    <DrawerPortal>
      <DrawerOverlay />
      <DrawerPrimitive.Content ref={ref} className={cn("fixed inset-y-0 right-0 z-20 border-none border-neutral-950 flex h-screen flex-col bg-neutral-950", className)} {...props}>
        <div className="mx-auto mt-4 h-2 w-full rounded-full bg-muted" />
        {children}
...
const DrawerHeader = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <div className={cn("grid gap-1.5 p-4 text-left sm:text-left", className)} {...props} />
)
