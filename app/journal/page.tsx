    ...
    const fetchPosts = async () => {
      try {
        ...
        const groupedPostsByYear = response.items.reduce((acc, post) => {
          ...
          const month = moment(post.fields.published).format("MMM");
        ...
        {Object.keys(groupedPosts)
          ...
          .map((year) => (
            <div key={year} className="pb-10">
              <h3 className={`text-sienna font-thin text-sm md:text-normal uppercase pb-2 pt-10`}>
                {year}
              </h3>
              <div className="grid grid-cols-3">
                <div>
                  {Object.keys(groupedPosts[year])
                    .sort((a, b) => moment(b, "MMM") - moment(a, "MMM")) // Sort months in reverse order
                    .map((month) => (
                      <div key={month} className="pb-12 xl:pb-5">
                        <h4 className={`uppercase text-sienna text-xl md:text-2xl font-extralight`}>
                          {month}
                        </h4>
                      </div>
                    ))}
